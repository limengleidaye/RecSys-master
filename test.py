...
class LshbasedUBCF:
    ...
    def read_data(self):
        ...
        rating_data = pd.read_table(filepath + "/ml-1m/ratings.dat", sep='::', header=None, names=rating_rnames,
    ...
    def build_datarating_matrix(self):
        ...
        movie_index1 = 0
        for user in self.train_user:
            self.user_index.setdefault(user, user_index1)
            user_index1 += 1
        for movie in self.train_movie:
            self.movie_index.setdefault(movie, movie_index1)
            movie_index1 += 1
        for user, movies in self.train_data.items():
            for movie, rating in movies.items():
                self.data.append([self.user_index[int(user)], self.movie_index[int(movie)], self.user_sim_matrix.setdefault(user, {})])
        self.user_num1 = user_index1
        self.movie_num1 = movie_index1
        self.rating_matrix_true = np.zeros((user_index1, movie_index1))
        self.rating_matrix_test = np.zeros((user_index1, movie_index1))
    ...
    def initLFM(self, K=13, i=1):
        ...
        for d in self.test_data1:
            self.rating_matrix_test[d[0]][d[1]] = d[2]
        #print(shape(self.rating_matrix_test))

        p = np.random.random((self.user_num1, K))
        q = np.random.random((self.movie_num1, K))
        p *= i
        q *= i
        return p, q
    ...
    def eva(self, k):
        ...
        self.rating_matrix_true
        self.predicted_rating_matrix
        #去除训练集中的原始，保留预测值
        for i in range(self.user_num1):
            for j in range(self.movie_num1):
        ...
        for i in range(self.user_num1):
            for j in range(self.movie_num1):
                if self.predicted_rating_matrix[i][j] != 0 and self.rating_matrix_test[i][j] != 0:
                    hit += 1
        precision = hit/(1.0 * self.k * self.user_num1)
        recall = hit/(1.0 * test_count)
        recall = hit / (1.0 * count)
...
def test_LshbasefUbcf():
    ...
    for n in n_set:
        ...
        precision, recall = lsh_ubcf.eva(k=5)
        print("%-18d%-19.5f%-20.5f%-19.5f%-20.5f%-10.5f" % (n, precision, recall, mae, rmse, time.time() - start))
